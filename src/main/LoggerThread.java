...
public class LoggerThread extends Thread
{
	...
	public LoggerThread(String threadName, String logDirectory)
	{
		...
		String logDir = logDirectory + date + time;
	 ...
	 * @throws InterruptedException
	 */
	public static void releaseAccess() throws InterruptedException
	{
		access.release();
	}
	...
	private void addToFile(LogItem li)
	{
		...
		if (!foundFile)
		{
			...
			String currentFiles = "Did not find a file for " + li.getClassName();
			currentFiles += "\nFiles currently in Vector<File>: ";
			for (File f : logFiles)
			{
				currentFiles += "\t\t" + f.getName();
			}
...
//			String log[] = {"Log File Manager", Calendar.getInstance().getTime().toString() + " " + currentFiles + "\n"};
			LogItem nli = new LogItem("Log File Manager", Calendar.getInstance().getTime().toString(), currentFiles, "Notice");
			addToList(nli);
